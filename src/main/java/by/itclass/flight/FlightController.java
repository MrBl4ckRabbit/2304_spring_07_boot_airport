...
public class FlightController {
    ...
    public String saveFlight(@RequestParam(name = "id", required = false) Integer id,
                             @RequestParam(name = "number") String number,
                             @RequestParam(name = "direction") String direction,
                             @RequestParam(name = "planeId") int planeId) {
        var plane = airplaneRepository.findById(planeId).get();
        var flight=id==null?new Flight(number, direction, plane):new Flight(id,number,direction,plane);
    ...
    public String viewFlight(
            @PathVariable(name = "id") int flightId, Model model) {
        model.addAttribute("flight", repository.findById(flightId).get());
        return "flight-info";
    }

    @GetMapping("/delFlight/{id}")
    public String delFlight(@PathVariable(name = "id") int flightId) {
        repository.deleteById(flightId);
        return "redirect:/allFlights";
    }

    @GetMapping("/updFlight/{id}")
    public String updFlight(@PathVariable(name = "id") int flightId,
                            Model model) {
